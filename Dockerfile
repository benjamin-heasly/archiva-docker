...
WORKDIR /opt/archiva
RUN wget "$(curl 'http://www.apache.org/dyn/closer.cgi' | grep -o '<strong>[^<]*</strong>' | sed 's/<[^>]*>//g' | head -1)archiva/2.2.1/binaries/apache-archiva-2.2.1-bin.tar.gz" && \
    md5sum apache-archiva-2.2.1-bin.tar.gz > apache-archiva-2.2.1-bin.tar.gz.my.md5 && \
    wget https://www.apache.org/dist/archiva/2.2.1/binaries/apache-archiva-2.2.1-bin.tar.gz.md5 && \
    cmp -s apache-archiva-2.2.1-bin.tar.gz.my.md5 apache-archiva-2.2.1-bin.tar.gz.md5 && \
    tar -xzf apache-archiva-2.2.1-bin.tar.gz && \
    rm apache-archiva-2.2.1-bin.tar.gz*
